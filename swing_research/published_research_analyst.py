...
OVERNIGHT_RETURN_ANOMALY = PublishedStrategy(
    ...
    scope_reductions=(
        ...
        "HOLDING PERIOD = 1 TRADING DAY (the shortest this program's Trade-based backtesting engine "
        "can express), combined with fill_timing=\"close_to_next_open\" (added 2026-09-06 to "
        "execution_realism_engine.py, purpose-built for this strategy) -- this now achieves a "
        "genuine buy-at-close/sell-at-next-open round trip: entry_price stays at that day's Close "
        "(correct, no substitution needed), and only exit_price is replaced, with exit_date's OWN "
        "Open (not the day after) -- since the 1-trading-day hold already makes exit_date the very "
        "next trading day after entry, this is exactly Close(entry_date) -> Open(exit_date). FIXED "
        "2026-09-06 after the original 2026-09-05 run (EXP-076, REJECT) used fill_timing="
        "\"next_day_open\" instead, which shifts BOTH legs to an Open price, spanning Open(t+1) to "
        "Open(t+2) -- one full extra intraday session the source papers say moves opposite the "
        "effect under test, contaminating that result. Re-run under the corrected mechanics."
    ),
    distinctiveness=(
    ...
    assumptions_impact=(
        ...
        "1-trading-day hold under close_to_next_open fills (see scope_reductions above): MINOR, "
        "a direct, faithful isolation of the paper's own overnight round trip as of the 2026-09-06 "
        "fix -- the prior next_day_open-based run's MATERIAL, LIKELY-UNDERSTATING contamination no "
        "longer applies."
    ),
)
