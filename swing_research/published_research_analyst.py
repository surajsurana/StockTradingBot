...
OVERNIGHT_RETURN_ANOMALY = PublishedStrategy(
    name="Overnight Return Anomaly",
    source_citation=(
        "Lou, D., Polk, C. and Skouras, S. (2019), \"A Tug of War: Overnight Versus Intraday "
        "Expected Returns,\" Journal of Financial Economics, Vol. 134, No. 1 (see also Berkman, "
        "D., Koch, P.D., Tuttle, L. and Zhang, S.J. (2012), \"Paying Attention: Overnight Returns "
        "and the Cross-Section of Stock Returns,\" Journal of Finance, Vol. 67, No. 5)."
    ),
    mechanism=(
        "A stock's TOTAL (Close-to-Close) return decomposes into an overnight (Close-to-Open) "
        "component and an intraday (Open-to-Close) component. The paper's central finding is a "
        "genuine 'tug of war': the overnight component PERSISTS (high past cumulative overnight "
        "return predicts high future overnight return -- attributed to sustained institutional/"
        "attention-driven buying pressure that clears mostly at the open), while the intraday "
        "component REVERSES over the same horizon -- two halves of the same total return moving in "
        "OPPOSITE directions. First strategy in this program built on a return COMPONENT rather "
        "than total return."
    ),
    rules=(
        "Daily overnight return_t = Open_t / Close_{t-1} - 1. Formation-period signal = cumulative "
        "(compounded) overnight return over a trailing formation window. Cross-sectional decile "
        "sort by this figure at each formation date. Long the TOP decile (strongest cumulative "
        "overnight return) -- the paper's own long-side persistence finding."
    ),
    variant_chosen=(
        "21-trading-day (1-month) formation window -- this program's own established convention "
        "for a short-horizon anomaly (identical choice to Short-Term Reversal, SW-008), not one "
        "single window uniquely prescribed by the source papers, which examine several horizons. "
        "See swing_research/cross_sectional.py's OVERNIGHT_RETURN_FORMATION_DAYS."
    ),
    scope_reductions=(
        ...
        "ORIGINAL METHODOLOGY AT ALL -- same disclosed pattern as every other strategy. "
        "*** THE MOST IMPORTANT DISCLOSED GAP: HOLDING PERIOD = 1 TRADING DAY (the shortest this "
        "program's Trade-based backtesting engine can express), combined with next-day-open fill "
        "timing (execution_realism_engine.py, same mechanism approved for Amihud, SW-010) -- this "
        "was expected to approximate a pure overnight-only round trip, but does NOT: shifting BOTH "
        "the entry and exit to an Open price means the realized hold spans Open(t+1) to Open(t+2), "
        "i.e. one FULL intraday session plus its flanking overnight moves, not overnight return in "
        "isolation. Per the source papers' own finding, that intraday session is expected to move "
        "in the OPPOSITE direction from the overnight-persistence effect under test -- so this "
        "implementation nets the effect against a same-magnitude, oppositely-signed contaminating "
        "return, rather than isolating it. This program's Trade-based engine (single entry_price/"
        "exit_price per trade, no intra-trade session accounting) cannot express a genuine "
        "buy-at-close/sell-at-next-open round trip without a session-aware execution model this "
        "program does not have -- disclosed as a real, un-worked-around limitation, not papered over."
    ),
    distinctiveness=(
        "First strategy in this program selecting on a RETURN COMPONENT (overnight only) rather "
        "than total return -- every existing strategy (momentum, reversal, MAX Effect, Amihud, "
        "Betting Against Beta, Idiosyncratic Volatility) is built on Close-to-Close total return or "
        "a risk/liquidity proxy derived from it. Needs zero new data (Open and Close are already "
        "columns in every OHLCV pull this program makes)."
    ),
    assumptions_impact=(
        "Long-only: DIRECTIONALLY UNKNOWN, standard. "
        "21-day formation window: MINOR-to-MODERATE -- a restatement into this program's own "
        "convention, not the paper's single prescribed window. "
        "Single-vintage holding: MODERATE-to-MATERIAL, DIRECTIONALLY UNKNOWN, same reasoning as "
        "every prior cross-sectional strategy. "
        "1-trading-day hold under next-day-open fills (see scope_reductions above): MATERIAL, "
        "LIKELY UNDERSTATES the pure overnight-only effect specifically, since the contaminating "
        "intraday-session return this implementation cannot avoid is expected to work directly "
        "against the effect under test -- this result should be read as a LOWER-BOUND test of the "
        "underlying mechanism, not a faithful isolation of it."
    ),
)
