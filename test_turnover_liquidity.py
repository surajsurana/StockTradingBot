"""
Unit tests for swing_research/strategies/turnover_liquidity.py -- hand-
constructed synthetic series exercising the turnover-percentile
...
percentile-based early exit. Mirrors test_max_effect.py's structure (the
sibling bottom-decile cross-sectional strategy). Run with:

    python test_turnover_liquidity.py
"""
...
from swing_research.strategies.turnover_liquidity import (
    HOLDING_PERIOD_CALENDAR_DAYS, TURNOVER_FORMATION_DAYS, TURNOVER_PERCENTILE_THRESHOLD,
    STOP_LOSS_PCT, TurnoverLiquidityStrategy,
)
...
class TestPrecompute(unittest.TestCase):
    def test_qualifies_when_percentile_at_or_below_threshold(self):
        ...
        df = _steady_series()
        df["turnover_percentile"] = 5.0   # bottom decile (illiquid stock)
    ...
    def test_does_not_qualify_above_threshold(self):
        strategy = TurnoverLiquidityStrategy()
        df = _steady_series()
        df["turnover_percentile"] = 10.1   # just above the bottom-decile cutoff
        precomputed = strategy.precompute(df)
        self.assertFalse(bool(precomputed["qualifies"].iloc[-1]))

    def test_missing_turnover_percentile_column_defaults_to_disqualified(self):
        strategy = TurnoverLiquidityStrategy()
        df = _steady_series()  # no turnover_percentile column at all
        precomputed = strategy.precompute(df)
        self.assertFalse(bool(precomputed["qualifies"].iloc[-1]))
    ...
    def test_exactly_at_threshold_qualifies(self):
        strategy = TurnoverLiquidityStrategy()
        df = _steady_series()
        df["turnover_percentile"] = TURNOVER_PERCENTILE_THRESHOLD
        precomputed = strategy.precompute(df)
        self.assertTrue(bool(precomputed["qualifies"].iloc[-1]))
...
class TestExitSignal(unittest.TestCase):
    ...
    def test_no_percentile_based_early_exit_exists(self):
        ...
        class Row:
            date = near_date
            Close = 150.0
            turnover_percentile = 99.0  # now firmly liquid (top decile) -- should NOT trigger an exit
        self.assertIsNone(strategy.exit_signal_at(Row(), self._position(entry_date)))
...
class TestConstants(unittest.TestCase):
    def test_documented_parameters_match_the_approved_plan(self):
        ...
        self.assertAlmostEqual(TURNOVER_PERCENTILE_THRESHOLD, 10.0)
        self.assertAlmostEqual(STOP_LOSS_PCT, 0.08)
        self.assertEqual(TurnoverLiquidityStrategy.max_units, 1)
        self.assertAlmostEqual(TurnoverLiquidityStrategy.risk_pct_per_unit, 0.01)
        self.assertEqual(TurnoverLiquidityStrategy.min_lookback_days, 21)

    def test_threshold_is_a_bottom_decile(self):
        self.assertLess(TURNOVER_PERCENTILE_THRESHOLD, 50.0)
