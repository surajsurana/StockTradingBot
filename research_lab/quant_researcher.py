...
def propose_hypotheses(api_key: str, n: int = 8, call_fn: Optional[Callable[[str], str]] = None,
    ...
    # than anything else that calls this function. This has now hit the
    # same "thinking block only, no text" failure three times as the
    # Knowledge Base + conclusions history grew (1024 -> 4096 -> 8192, each
    # insufficient) -- since this prompt only gets LONGER as more
    # experiments and conclusions accumulate, jumping substantially higher
    # here rather than incrementing again, to avoid a fourth recurrence.
